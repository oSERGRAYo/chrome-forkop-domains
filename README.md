...
| UCI-опция доменов | `domain` | имя листа в секции (`domain` — точное совпадение, `domain_suffix` — вместе с поддоменами) |
...
подхватывает конфиг на лету — процесс перезапускается, и активные соединения
через прокси на несколько секунд рвутся. Полностью бесшовно — никак.
